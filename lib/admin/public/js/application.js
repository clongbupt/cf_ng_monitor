...
var Application =
{
    ...
    {
        this.spacesTable = this.createTable(this.ID__SPACES, this.spaceClicked, [[1, "desc"]]);
    },
    ...
    getHealthManagersRow: function(row, healthManager)
    {
        ...
        if (healthManager.connected)
        {
            ...
            // as there is no those info in health_manager_ng, so comment it 
            /* 
        ...
        if (dropletExecutionAgent.connected)
        {
            ...
            else
            {
                ...
                if (dropletExecutionAgent.data.instance_registry == undefined)
                    ...
                    var apps_num = 0;
                    for(var i in dropletExecutionAgent.data.instance_registry)
                        apps_num ++;
                    row.push( (apps_num) );
                }
                row.push( (dropletExecutionAgent.data.mem_used_bytes + dropletExecutionAgent.data.mem_free_bytes) / (1024*1024) );
                row.push( (dropletExecutionAgent.data.mem_used_bytes + dropletExecutionAgent.data.mem_free_bytes) / (1024*1024) );
                row.push( dropletExecutionAgent.data.mem_free_bytes / (1024*1024) );
                row.push( dropletExecutionAgent.data.available_memory_ratio);
                row.push( dropletExecutionAgent.data.mem_used_bytes / (1024*1024) );
                // row.push(0);
                // row.push(0);
                // row.push(0);
                // row.push(0);
    ...
    {
        // Comment health_manager as there is not any application info, 
        // we cloud only get application info from dea 
        ...
        */
        ...
        this.getData(this.URL__APPLICATIONS, reload).done(function(result)
        {
            console.log("=========applications============");
            console.log(result);
            Application.refreshTab(Application.ID__APPLICATIONS, [result], reload);
        });
    },

    getApplicationsTableData: function(results)
    {
        return this.getTableData(results, this.ID__APPLICATIONS);
    },

    getApplicationsRow: function(row, application)
    {
        row.push(application.name);
        if (application.state == "STARTED" && application.package_state == "STAGED"){
            row.push("STARTED");
            row.push(application.created_at);
            ...
            row.push(application.space_name)
        }
            
        else{
            row.push("STOPPED");
            this.addEmptyElementsToArray(row,6);
        }

        row.push(application);
    },
    ...
    */
    ...
    {
        Application.itemClicked("Application", 8, true);
    },
    ...
    handleApplicationClicked: function(table, app, row)
    {
        if (app.state == "STARTED" && app.package_state == "STAGED")
        {
            ...
            Application.addStatusRow(table, "Package State", app.package_state);

            Application.addPropertyRow(table, "Buildpack",       Application.formatString(app.detected_buildpack));
            Application.addPropertyRow(table, "Started",         Utilities.formatDateString(app.created_at));
            Application.addPropertyRow(table, "Memory Reserved", Utilities.formatNumber(app.memory));
            Application.addPropertyRow(table, "Disk Reserved",   Utilities.formatNumber(app.disk_quota));
            Application.addPropertyRow(table, "Disk Reserved",   Utilities.formatNumber(app.file_descriptors));
            Application.addPropertyRow(table, "Instances",     Application.formatString(app.instances));
            Application.addPropertyRow(table, "Space Name",   Application.formatString(app.space_name));
    ...
    //// Comment some codes as the new interface of dea does not represent those info. 
    /*
    handleApplicationClicked: function(table, app, row)
    {
    ...
    */
    ...
    refreshUsersPage: function(reload)
    {
        ...
        this.getData(this.URL__USERS, reload).done(function(result)
        {
            console.log("=============Users================");
            console.log(result);

            Application.refreshTab(Application.ID__USERS, [result], reload);
        });

        // there is no user info from health_manager and dea, so we have to fetch info from database
    ...
    {
        return this.getTableData(results, this.ID__USERS);
    },

    // Comment codes below the line, because we get user info from database, 
        // so no need to handle the info frm health_manager and dea
    ...
    */
    ...
    // Add this function to push the valid info into each row
    getUsersRow: function(row, user)
    {
        row.push(user.email);
        row.push(user.username);
        row.push(user.created);
        row.push(user.lastmodified);
        row.push(user.version);
        row.push(user.phonenumber);
        row.push(user.authorities);
        row.push(user.active);
        row.push(user.org_name);
    ...
    {
        Application.itemClicked("User", 9, true);
    },
    ...
    handleUserClicked: function(table, user, row)
    {
        ...
        Application.addPropertyRow(table, "Username", Application.formatString(user.username));

        Application.addPropertyRow(table, "Create Time",             Utilities.formatDateString(user.created));
        Application.addPropertyRow(table, "Last Modified Time", Utilities.formatDateString(user.lastmodified));
        Application.addPropertyRow(table, "Version",     Utilities.formatNumber(user.version));
        Application.addPropertyRow(table, "Phone Number",     Application.formatString(user.phonenumber));
        Application.addPropertyRow(table, "Family Name",   Application.formatString(user.familyname));
        Application.addPropertyRow(table, "Given Name",       Application.formatString(user.givenname));
        Application.addPropertyRow(table, "Authorities",       Application.formatString(user.authorities));
        Application.addPropertyRow(table, "Active",       Application.formatStatus(user.active));
    ...
    refreshSpacesPage: function(reload)
    {
        ...
        this.getData(this.URL__SPACES, reload).done(function(result)
        {
            console.log("===========spaces===============");
            console.log(result);
            Application.refreshTab(Application.ID__SPACES, [result], reload);
        });
    },

    getSpacesTableData: function(results)
    {
        return this.getTableData(results, this.ID__SPACES);
    },

    getSpacesRow: function(row, space)
    {

        row.push(space.name);
        row.push(space.org_name);
        row.push(space.created_at);
        row.push(space.running_apps);
        row.push(space.stopped_apps);
        row.push(space.unknown_apps);
    ...
    {
        Application.itemClicked("Space", 6, false);
    },

    handleSpaceClicked: function(table, space, row)
    {
        Application.addPropertyRow(table, "Name",              space.name, true);
        Application.addLinkRow(table,     "Organization Name",               Application.formatString(space.org_name));
        Application.addPropertyRow(table, "Started Time",           Application.formatDateString(space.created_at), true);
        Application.addPropertyRow(table, "Running Apps",            Application.formatNumber(space.running_apps));
        Application.addPropertyRow(table, "Stopped Apps",             Application.formatNumber(space.stopped_apps));
        Application.addPropertyRow(table, "Unknown Apps",               Application.formatNumber(space.unknown_apps));
    },
